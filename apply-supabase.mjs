...
async function runSql(label, file) {
  ...
  if (!r.ok) {
    console.error(`✗ ${label} (${file}): ${r.status} ${text.slice(0, 400)}`);
    return false;
  }
...
if (!proj.ok) {
  console.error(`Cannot access project: ${proj.status} ${await proj.text()}`);
  process.exit(1);
}
...
if (!keysRes.ok) {
  console.error(`Cannot read keys: ${keysRes.status} ${await keysRes.text()}`);
  process.exit(1);
}
