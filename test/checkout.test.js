...
test('checkout: donation below $1 is rejected', async () => {
  ...
  try {
    const r = await onRequestPost({
      request: fakeRequest({ body: { type: 'donation', amount_cents: 50 } }), env: fakeEnv(),
    });
    assert.equal(r.status, 400);
...
test('checkout: valid donation creates a session and inserts into donations', async () => {
  ...
  try {
    const r = await onRequestPost({
      request: fakeRequest({ body: { type: 'donation', amount_cents: 2500, name: 'Pat', email: 'p@x.com' } }),
      env: fakeEnv(),
    });
    ...
    assert.equal(new URLSearchParams(stripeCall.options.body).get('line_items[0][price_data][unit_amount]'), '2500');
...
test('checkout: unknown membership tier -> 400', async () => {
  ...
  try {
    const r = await onRequestPost({
      request: fakeRequest({ body: { tier_id: 'ghost' } }), env: fakeEnv(),
    });
    assert.equal(r.status, 400);
