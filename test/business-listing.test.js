...
test('business-listing: name and email are required', async () => {
  const fetch = mockFetch(() => ({}));
  try {
    const r = await onRequestPost({ request: fakeRequest({ body: { name: 'Acme' } }), env: fakeEnv() });
    assert.equal(r.status, 400);
...
test('business-listing: honeypot _hp silently accepted, no DB write', async () => {
  ...
  try {
    const r = await onRequestPost({
      request: fakeRequest({ body: { name: 'Acme', email: 'a@x.com', _hp: '1' } }), env: fakeEnv(),
    });
...
test('business-listing: defaults category to "other" and approved to false', async () => {
  ...
  try {
    const r = await onRequestPost({
      request: fakeRequest({ body: { name: 'Acme', email: 'a@x.com', description: 'we <sell> things' } }),
      env: resendEnv(),
    });
    assert.equal(r.status, 200);
